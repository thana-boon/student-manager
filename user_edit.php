...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $newPassword2 = (string)($_POST['new_password2'] ?? '');
    ...
    if ($newPassword !== '' && strlen($newPassword) < 6) {
        $errors[] = 'รหัสผ่านใหม่ต้องอย่างน้อย 6 ตัวอักษร (หรือเว้นว่างเพื่อไม่เปลี่ยน)';
    }
    ...
    if (count($errors) === 0) {
        if (users_username_taken($pdo, $username, $id)) {
            $errors[] = 'ชื่อผู้ใช้นี้ถูกใช้แล้ว';
        } else {
          users_update($pdo, $id, $username, $displayName);
            if ($newPassword !== '') {
                users_update_password($pdo, $id, $newPassword);
            }
            header('Location: users.php');
            exit;
        }
...
<main class="mx-auto max-w-3xl px-4 py-8">
  <div class="mb-6 flex items-start justify-between gap-4">
    <div>
      <h2 class="text-2xl font-bold tracking-tight text-slate-900">แก้ไขผู้ใช้</h2>
      <p class="mt-1 text-sm text-slate-500">อัปเดตชื่อผู้ใช้/สิทธิ์ และตั้งรหัสผ่านใหม่ (ถ้าต้องการ)</p>
    </div>
  ...
  <form method="post" class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    ...
    <div class="grid gap-4">
      <div>
        ...
        <input name="display_name" value="<?= htmlspecialchars($displayName, ENT_QUOTES) ?>"
      ...
      <div>
        ...
        <input name="username" value="<?= htmlspecialchars($username, ENT_QUOTES) ?>"
          class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-slate-900 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10" />
      </div>

      <div>
        <label class="mb-1.5 block text-sm font-medium text-slate-700">รหัสผ่านใหม่ (ถ้าต้องการเปลี่ยน)</label>
        <input type="password" name="new_password" placeholder="เว้นว่างเพื่อไม่เปลี่ยน"
      ...
      <div>
        <label class="mb-1.5 block text-sm font-medium text-slate-700">ยืนยันรหัสผ่านใหม่</label>
        <input type="password" name="new_password2" placeholder="พิมพ์รหัสผ่านใหม่ซ้ำ"
          class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-slate-900 placeholder:text-slate-400 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10" />
      </div>
    </div>
