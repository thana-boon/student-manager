...
$classRoom = trim((string)($_POST['class_room'] ?? ''));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $error === '') {
    ...
    if ($currentYear === null) {
        $errors[] = 'กรุณาตั้งปีการศึกษาปัจจุบันก่อน';
    }
    ...
    if (count($errors) === 0) {
        try {
            ...
            $id = students_create($pdoSchool, $m, (int)$currentYear['id'], (string)$currentYear['name'], [
            ...
            app_log('student.create', ['id' => $id, 'year_id' => (int)$currentYear['id']]);
...
<main class="mx-auto max-w-3xl px-4 py-8">
  <div class="mb-6 flex items-start justify-between gap-4">
    <div>
      <h2 class="text-2xl font-bold tracking-tight text-slate-900">เพิ่มนักเรียน</h2>
      <p class="mt-1 text-sm text-slate-500">เพิ่มนักเรียนในปีการศึกษา: <span class="font-semibold text-slate-700"><?= htmlspecialchars((string)($currentYear['name'] ?? '-'), ENT_QUOTES) ?></span></p>
    </div>
    ...
    <div class="grid gap-4">
      ...
      <?php if ($mapping !== null && (string)($mapping['full_name'] ?? '') !== ''): ?>
        <div>
          ...
          <input name="full_name" value="<?= htmlspecialchars($fullName, ENT_QUOTES) ?>" placeholder="เช่น เด็กชายสมชาย ใจดี"
            class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-slate-900 placeholder:text-slate-400 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10" />
        </div>
      <?php else: ?>
        <div class="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <input name="first_name" value="<?= htmlspecialchars($firstName, ENT_QUOTES) ?>" placeholder="เช่น สมชาย"
              class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-slate-900 placeholder:text-slate-400 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10" />
          </div>
          <div>
            <label class="mb-1 block text-sm text-slate-700">นามสกุล</label>
            <input name="last_name" value="<?= htmlspecialchars($lastName, ENT_QUOTES) ?>" placeholder="เช่น ใจดี"
              class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-slate-900 placeholder:text-slate-400 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10" />
          </div>
        </div>
      <?php endif; ?>
      ...
      <?php if (count($extraColumns) > 0): ?>
        <div class="mt-2 rounded-xl border border-slate-200 bg-slate-50 p-4">
          ...
          <p class="mt-1 text-xs text-slate-600">ฟิลด์ด้านล่างถูกดึงจากคอลัมน์ที่มีอยู่ในตารางนักเรียน เพื่อให้เพิ่มข้อมูลที่ถูกเก็บไว้ได้ครบ</p>
  ...
  <?php if ($currentYear === null && $error === ''): ?>
    <div class="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-xs text-amber-900">
      ยังไม่ได้ตั้งปีปัจจุบันของระบบนี้ ไปที่ <a class="underline" href="academic_years.php">หน้า Academic Years</a> เพื่อเลือกปีปัจจุบันก่อน
    </div>
  <?php endif; ?>
</main>
